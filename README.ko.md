...
> **상태: 개발 중.** 세 OS에서 실사용 중이지만 아직 정식 릴리스 전입니다.
> 내장된 Google 클라이언트가 테스트 상태일 수 있습니다 —
> [Google 계정](#google-계정) 참고.
...
내장된 OAuth 클라이언트는 `drive.file` 스코프(이 앱이 만든 파일만 봄)로
제한됩니다. 이 클라이언트가 **테스트** 상태인 동안에는 테스트 사용자로 추가된
계정만 로그인할 수 있습니다. 포크해서 재배포한다면 자체 클라이언트를 등록해
`BOTTARI_CLIENT_ID` / `BOTTARI_CLIENT_SECRET`로 지정하세요.
